...
DEFAULT_TRAFFIC_DATE_BY_FOLDER = {
    "March_7": "2026-03-07",
    "March_9": "2026-03-09",
    "January_10": "2026-01-10",
    "January_12": "2026-01-12",
    "July_12": "2025-07-12",
    "July_14": "2025-07-14",
}
...
def main():
    ...
    default_traffic_date = DEFAULT_TRAFFIC_DATE_BY_FOLDER.get(DATE_ROOT.name)
    if default_traffic_date is None:
        valid_folders = ", ".join(sorted(DEFAULT_TRAFFIC_DATE_BY_FOLDER))
        raise ValueError(
            f"Cannot infer traffic date from {DATE_ROOT.name}. "
    ...
    parser.add_argument(
        "--traffic-date",
        default=default_traffic_date,
        help="Configured St. Louis traffic date to use. Defaults to the date folder.",
    )
    parser.add_argument(
        "--step",
        type=float,
