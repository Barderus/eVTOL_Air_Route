"""Build St. Louis barycentric route-weight visualizations."""
...
MAPS_FOLDER = DATE_ROOT / "maps" / "barycentric"
...
<head>
  ...
  <title>St. Louis Route Weight Barycentric Tetrahedron</title>
...
def make_route_pair_plot(route_runs, route_pair, output_html):
    ...
    traffic_date = subset["traffic_date"].iloc[0]
    ...
    layout = {
        "title": {
            "text": (
                "St. Louis Route Weight Barycentric Tetrahedron - "
                f"{route_label} ({traffic_dataset}, {traffic_date})"
            ),
            "x": 0.5,
            "xanchor": "center",
        },
...
def main():
    """Save barycentric coordinates and one plot per St. Louis route pair."""
    if not ROUTE_RUNS_CSV.exists():
        raise FileNotFoundError(
            f"Run St. Louis weighted routes first: {ROUTE_RUNS_CSV}"
        )
    ...
    if route_runs.empty:
        raise ValueError("No successful St. Louis route runs found.")
