"""Select one representative route for each full-route cluster."""
...
REPRESENTATIVES_GEOJSON = os.path.join(
    OUTPUT_FOLDER,
    "all_cluster_representative_routes.geojson",
)


def load_inputs():
    """Load routes, clusters, and the similarity matrix."""
    for file_path in [
        ALL_ROUTES_GEOJSON,
...
def select_representatives(routes, similarity_matrix):
    """Select the medoid route for every cluster."""
    representative_rows = []
...
def main():
    """Save one representative route per full-route cluster."""
    ...
    print(f"Saved cluster representative CSV: {REPRESENTATIVES_CSV}")
    print(f"Saved cluster representative GeoJSON: {REPRESENTATIVES_GEOJSON}")
    print(
        representatives[
            [
                "cluster_id",
