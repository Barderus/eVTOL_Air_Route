"""Create an HTML map for all route clusters at threshold 0.15."""
...
ALL_ROUTE_CLUSTERS_CSV = "Chicago/route_robustness/output/cluster_assignments/all_route_clusters_threshold_015.csv"
OUTPUT_FOLDER = "Chicago/route_robustness/maps/route_clusters"
...
<head>
  ...
  <title>Route Robustness Clusters Threshold 0.15</title>
  <link
    rel="stylesheet"
...
<body>
  ...
  <script>
    ...
    legend.onAdd = function () {{
      ...
      div.innerHTML = `<h1>Route Clusters Threshold 0.15</h1><div>${{rows}}</div>`;
      return div;
    }};
    legend.addTo(map);
  </script>
...
def load_clustered_routes():
    """Join all route geometries with 0.15-threshold cluster labels."""
    for file_path in [ALL_ROUTES_GEOJSON, ALL_ROUTE_CLUSTERS_CSV]:
    ...
    return routes.merge(clusters[cluster_columns], on="route_run_id", how="left")
...
def main():
    """Write the 0.15-threshold cluster map HTML."""
    ...
    print(f"Saved 0.15 cluster map: {OUTPUT_HTML}")
