"""Create an HTML map for cluster representative routes."""
...
REPRESENTATIVES_GEOJSON = "Chicago/route_robustness/output/representatives/all_cluster_representative_routes.geojson"
OUTPUT_FOLDER = "Chicago/route_robustness/maps/representatives"
...
<head>
  ...
  <title>Route Robustness Cluster Representative Routes</title>
  <link
    rel="stylesheet"
...
<body>
  ...
  <script>
    ...
    legend.onAdd = function () {{
      ...
      div.innerHTML = `
        <h1>Representative Cluster Routes</h1>
        <div>${{rows}}</div>
      `;
      return div;
    }};
...
def main():
    """Write the representative cluster route map HTML."""
    ...
    cluster_colors = build_cluster_colors(
        representatives["cluster_id"].dropna().unique()
    )
    ...
    print(f"Saved cluster representative map: {OUTPUT_HTML}")
